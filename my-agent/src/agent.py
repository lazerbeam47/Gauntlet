...
class Assistant(Agent):
    def __init__(self, instructions: str | None = None) -> None:
        super().__init__(
            ...
            instructions=instructions or textwrap.dedent(
                """\
...
def target_instructions(job_metadata: str) -> str | None:
    """Use the dashboard prompt for this dispatched test, when supplied."""
    try:
        prompt = json.loads(job_metadata or "{}").get("agent_prompt", "").strip()
    except json.JSONDecodeError:
        logger.warning("Ignoring malformed target dispatch metadata")
        return None
...
async def my_agent(ctx: JobContext):
    ...
    print({
        ...
        "attributes": dict(ctx.room.local_participant.attributes),
    })
    ...
    await session.start(
        ...
        room_options=room_io.RoomOptions(
            ...
            # so it has LiveKit's AGENT participant kind rather than the
            # default STANDARD/SIP kinds consumed by RoomIO.
